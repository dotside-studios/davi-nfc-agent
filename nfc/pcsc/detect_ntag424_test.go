...
// and records what it was asked. Anything not in the table is answered "class
// not supported", as a card does for a command it does not implement.
...
// shortISO4ATR is the ATR a Type 4 card presents on a typical reader: the short
// ISO 14443-4 form, with no card-type byte. Nothing in it says which card this
// is — DetectTagTypeFromATR reads it as unknown and isISO14443_4Compatible
// rejects it — which is exactly why the version probe exists.
...
// NXP vendor, NTAG product type, major 0x30, the 504-byte storage code and the
// ISO 14443-4 protocol byte, answered 91 AF because two frames follow.
...
// one. Without the probe this card is refused outright: its ATR names nothing,
// and neither ATR check in this package accepts it, so detection would end in an
// unsupported-tag error rather than in a Type 4 driver.
...
// The probe names only what it can name. A card that does not implement the
// command, or answers as something else, is left to the detection that follows
// rather than being called an NTAG 424.
...
// A kind the ATR already named is not probed. The card is asked nothing, so the
// common cards pay nothing for a check that only Type 4 cards need.
