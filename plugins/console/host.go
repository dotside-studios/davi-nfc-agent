...
//
// It reaches the agent and nothing else. An action taken in the console puts
// the agent in a state, and the tray follows that state the same way it follows
// one of its own — there is no path from here into whatever is drawing.
...
func (h *webuiHost) StopAgent() { h.agent.Stop() }

func (h *webuiHost) QuitAgent() { h.agent.Quit() }
...
func (h *webuiHost) SelectDevice(devicePath string) error {
	...
	return h.agent.SelectDevice(devicePath)
}
...
func (h *webuiHost) InstallCA() error {
	...
	// The tray offers the same action and has nothing left to offer now. It
	// hears about it the way it hears about everything else.
	h.agent.PublishState()
	return nil
}
