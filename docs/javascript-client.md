...
A universal JavaScript library for connecting to the agent as an NFC **device**
— a phone or Node process acting as a reader on the agent's behalf. Works in
Node.js and the browser, and is NFC-source agnostic: call `scanTag()` when
whatever NFC library you use detects a tag.

> **Looking for the client role?** An application that *consumes* tags from the
> agent and writes to them uses `NFCClient`, documented in
> [docs/client/](client/). Both roles share the agent's port and nothing else.
