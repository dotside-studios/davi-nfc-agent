...
// Spec is the state a menu item is created with. Platforms fix some of it at
// creation, whether an item is a checkbox most of all, so it is passed in one
// go rather than set afterwards.
type Spec struct {
	Title    string
	Tooltip  string
	Checkbox bool
...
// driver may type-assert its own type back out of the interface.
