...
// errNoHolder is reported by an operation on a held tag when no backend beneath
// this manager holds tags, which is a build whose devices are all polled
// through a reader.
...
//
// A signal of this manager's own, with the child's connected to it in New.
// Handing back the child's would return nil for a child that reports no scans,
// and [nfc.OnScan] would find this type satisfying [nfc.TagReporter] and call
// Connect on that nil.
...
// The tag router's questions, forwarded whole. [nfc.TagHolder] is satisfied
// all-or-nothing: implementing part of it leaves [nfc.TagsHeldBy] reporting
// that nothing here holds a tag, and every operation on a tag a device reported
// fails as though the tag were gone.
...
// Pinned so a partial forward fails to compile.
