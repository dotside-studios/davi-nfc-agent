// Package event carries values from one producer to any number of subscribers.
//
// A [Signal] is a registration point for callbacks: connect a handler and get a
// [Connection] back that removes it again. It is what a library publishes its
// notifications on, such as menu clicks or agent state changes.
package event
...
// Emit calls them all.
//
//	conn := agent.Events().State.Connect(func(s agent.State) { log.Println(s) })
