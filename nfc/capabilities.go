...
func InferTagCapabilities(tagType string) TagCapabilities {
	...
	case strings.Contains(tagTypeLower, "ntag424") || strings.Contains(tagTypeLower, "ntag 424"):
		// An NTAG 424 DNA carries the NTAG name but is a Type 4 card, driven by
		// APDU rather than by page. Matched before the NTAG21x case below,
		// which does not recognise it and would leave it to the conservative
		// default.
		caps.CanWrite = true
...
//   - When Capabilities().SupportsNDEF is false, ReadData() either returns raw
//     bytes or declines with a no-payload error. Returning nothing and no error
//     is indistinguishable from a successful read of an empty tag.
//
...
func AssertCapabilitiesConsistent(tag Tag) error {
	...
	if !caps.SupportsNDEF {
		...
		if err == nil && len(data) == 0 {
			return fmt.Errorf("capability drift: Capabilities().SupportsNDEF=false but ReadData() returned no bytes and no error; a tag with nothing to return must decline with a no-payload error")
		}
	}
